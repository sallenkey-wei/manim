...
class FourierCirclesScene(Scene):
    ...
    def __init__(self, **kwargs):
        ...
        self.parametric_function_step_size = None
    ...
    def get_vector_sum_path(self, vectors, color=YELLOW):
        ...
        #ParametricFunction类就是三维空间中的 参数曲线类，所以path可以直接在三位空间中显示
    ...
    def get_drawn_path(self, vectors, stroke_width=None, **kwargs):
        ...
        #将要画的轨迹分成很多段，这样可以每段设置不同的width
        broken_path = CurvesAsSubmobjects(path)
        broken_path.curr_time = 0

        def update_path(path, dt):
            # alpha = path.curr_time * self.get_slow_factor()
            #获取当前作画的时间点（0~1s之间）， 对应一个周期不同的位置
            ...
            for a, sp in zip(np.linspace(0, 1, n_curves), path):
                ...
                if b < 0:
                    width = 0
                else:#设置已经画过的轨迹逐渐隐去的效果，总体效果实现刚画完的部分宽width明显，画完时间越长的部分width越不明显,详见视频效果
...
class FourierOfPiSymbol(FourierCirclesScene):
    ...
    def add_vectors_circles_path(self):
        ...
        if self.start_drawn:
            #True:开始话图之前就把轨迹的轮廓画出来，False:开始之前不把轮廓画出来
            #另外increment_value设成其他值可以改变开始画图的起始点, 例如设成0.5试一试
    ...
    def set_decreasing_stroke_widths(self, circles):
        ...
        #作者本来的意图是 想让圆的轨迹宽度越来越细的，但是用min函数的话，很多圆细的看不见了
